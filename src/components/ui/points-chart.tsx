...
function PointsChart({ data, height = 260, title = "Your points", headerRight, yAxisLabel, levels, className, ...props }: PointsChartProps) {
  const yDomain = React.useMemo<[number, number]>(() => {
    const values = [...data.map((item) => item.total), ...(levels?.map((level) => level.value) ?? [])]
  ...
  return (
    <div className={cn("rounded-2xl border bg-card p-4", className)} {...props}>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 12, right: 12, left: 0, bottom: 4 }}>
            ...
            <YAxis
              ...
              label={
                yAxisLabel
                  ? { value: yAxisLabel, angle: -90, position: "insideLeft", fill: "var(--muted-foreground)", fontSize: 12, dx: -18 }
                  : undefined
              }
            />
            {levels?.map((level) => (
              <ReferenceLine
                ...
                label={{
                  ...
                  content: (labelProps: { viewBox?: unknown }) => (
                    <LevelReferenceStarLabel viewBox={(labelProps.viewBox as { x?: number; y?: number } | null) ?? null} color={level.color} />
                  ),
                }}
              />
            ))}
            <Tooltip
              ...
              content={({ active, payload, label }) => {
                ...
                return (
                  <div className="rounded-lg border bg-popover px-3 py-2 text-sm text-popover-foreground shadow-md" style={{ borderColor: "var(--border)" }}>
            ...
            />
            <Line type="monotone" dataKey="total" stroke="var(--primary)" strokeWidth={2} connectNulls dot={{ r: 3, fill: "var(--primary)" }} activeDot={{ r: 6 }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
